...
async function main() {
  ...
  if (existingCount > 0 && process.env.FORCE_SEED !== 'true') {
    console.log(`[Seed] Database already contains ${existingCount} invoices. Skipping seed to preserve data.`);
    return;
  }

  console.log('[Seed] Seeding sample invoice data...');
  ...
  const inv7 = await prisma.invoice.create({
    data: {
      ...
      taxDepartment: 'CỤC THUẾ TP. HÀ NỘI',
      taxAuthorityCode: '00E26TAA88123407',
      sellerName: 'CÔNG TY CỔ PHẦN CÔNG NGHỆ VÀ TRUYỀN THÔNG ALPHA',
      sellerTaxCode: '0101234567',
      sellerAddress: 'Tầng 3, Tòa nhà Alpha, 123 Đường Công Nghệ, Cầu Giấy, Hà Nội',
      sellerPhone: '024 3838 9999',
